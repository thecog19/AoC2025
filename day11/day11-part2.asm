...
; Day 11 Part 2: Count paths from "svr" to "out" visiting both "dac" and "fft"
; State-based memoization: count[node][state] where state = d*2 + f
; d=1 if dac visited, f=1 if fft visited
; state: 0=neither, 1=fft only, 2=dac only, 3=both
...
.init_hash:
    ...
    ; Initialize path_count to -1 (uncomputed) - 4 states per node
    lea rdi, [path_count]
    mov rcx, MAX_NODES * 4
    mov rax, -1
.init_paths:
...
.parsing_done:
    ...
    ; Compute initial state for svr
    ; state = d*2 + f where d=1 if svr==dac, f=1 if svr==fft
    xor rsi, rsi                    ; state = 0
    mov rax, [svr_idx]
    cmp rax, [dac_idx]
    jne .check_fft_start
...
.call_count:
    ; Call count_paths(svr_idx, state)
    mov rdi, [svr_idx]
    call count_paths
...
; count_paths(node_idx in rdi, state in rsi)
; state = d*2 + f where d=1 if dac visited, f=1 if fft visited
...
count_paths:
    ...
    mov r15, rsi                    ; r15 = state

    ; Check if at "out"
    cmp r12, [out_idx]
    jne .not_out

    ; At out: return 1 if state==3 (both visited), else 0
    cmp r15, 3
    jne .return_zero
    mov rax, 1
    jmp .return_val
...
.not_out:
    ; Check cache: index = node * 4 + state
    mov rax, r12
    shl rax, 2
    add rax, r15
    mov rax, [path_count + rax*8]
    cmp rax, -1
    jne .return_val
...
path_count: resq MAX_NODES * 4      ; memoized path counts (4 states per node)
