extern print_number
extern read_file
extern input_buffer

section .text
global _start

_start:
    mov rdi, filename
    call read_file
    mov [file_size], rax

    ; Find line width (position of first newline + 1)
    mov rdi, input_buffer
    xor rcx, rcx
.find_newline:
    movzx rax, byte [rdi + rcx]
    cmp al, 10
    je .found_newline
    cmp al, 13
    je .found_newline
    inc rcx
    jmp .find_newline
.found_newline:
    mov [line_width], rcx

    ; Check for CRLF
    movzx rax, byte [rdi + rcx]
    cmp al, 13
    jne .not_crlf
    inc rcx                      ; skip CR
.not_crlf:
    inc rcx                      ; skip LF
    mov [line_stride], rcx       ; stride includes newline chars

    ; Count number of lines (handle file not ending with newline)
    mov rax, [file_size]
    xor rdx, rdx
    div rcx
    mov [num_lines], rax
    ; If there's a remainder, there's one more line
    cmp rdx, 0
    je .lines_counted
    inc qword [num_lines]
.lines_counted:

    ; r15 = grand total
    xor r15, r15

    ; r12 = current column
    xor r12, r12

.process_columns:
    mov rax, [line_width]
    cmp r12, rax
    jge .done

    ; Check if this column is a separator (all spaces)
    call is_separator_column
    cmp rax, 1
    je .next_column

    ; Found start of a problem - find the end
    mov r13, r12                 ; r13 = start column
.find_end:
    inc r12
    mov rax, [line_width]
    cmp r12, rax
    jge .found_problem_end
    call is_separator_column
    cmp rax, 0
    je .find_end

.found_problem_end:
    ; Problem spans columns r13 to r12-1
    ; Get operator from last row
    mov rax, [num_lines]
    dec rax
    mov rcx, [line_stride]
    imul rax, rcx
    add rax, input_buffer
    add rax, r13

    ; Find operator in this problem's section of last row
    mov rbx, r12
    sub rbx, r13                 ; rbx = width of problem
.find_op:
    movzx rdx, byte [rax]
    cmp dl, '*'
    je .got_mult
    cmp dl, '+'
    je .got_add
    inc rax
    dec rbx
    jnz .find_op
    jmp .next_column            ; no operator found?

.got_mult:
    mov byte [current_op], '*'
    jmp .parse_numbers
.got_add:
    mov byte [current_op], '+'

.parse_numbers:
    ; Initialize accumulator based on operation
    cmp byte [current_op], '*'
    jne .init_add
    mov qword [accumulator], 1
    jmp .parse_rows
.init_add:
    mov qword [accumulator], 0

.parse_rows:
    ; Parse numbers from each row (except last)
    xor r14, r14                 ; r14 = current row

.row_loop:
    mov rax, [num_lines]
    dec rax                      ; skip operator row
    cmp r14, rax
    jge .problem_done

    ; Get pointer to start of this row's problem section
    mov rax, r14
    mov rcx, [line_stride]
    imul rax, rcx
    add rax, input_buffer
    add rax, r13                 ; rax = start of problem in this row

    mov rcx, r12
    sub rcx, r13                 ; rcx = width to scan

    ; Parse number from this section
    call parse_number_in_range
    cmp rbx, 0                   ; rbx = 1 if number found
    je .next_row

    ; Apply operation
    cmp byte [current_op], '*'
    jne .do_add
    mov rcx, [accumulator]
    imul rax, rcx
    mov [accumulator], rax
    jmp .next_row
.do_add:
    add [accumulator], rax

.next_row:
    inc r14
    jmp .row_loop

.problem_done:
    mov rax, [accumulator]
    add r15, rax
    jmp .process_columns

.next_column:
    inc r12
    jmp .process_columns

.done:
    mov rdi, r15
    call print_number

    mov rax, 60
    xor rdi, rdi
    syscall

; Check if column r12 is all spaces
; Returns 1 in rax if separator, 0 otherwise
is_separator_column:
    push r14
    xor r14, r14                 ; row counter
.check_row:
    mov rax, [num_lines]
    cmp r14, rax
    jge .is_sep

    mov rax, r14
    mov rcx, [line_stride]
    imul rax, rcx
    add rax, r12
    add rax, input_buffer
    movzx rax, byte [rax]
    cmp al, ' '
    jne .not_sep
    inc r14
    jmp .check_row
.is_sep:
    mov rax, 1
    pop r14
    ret
.not_sep:
    xor rax, rax
    pop r14
    ret

; Parse number from range starting at rax with width rcx
; Returns number in rax, rbx=1 if found, rbx=0 if not
parse_number_in_range:
    push r8
    push r9
    mov r8, rax                  ; r8 = current pos
    mov r9, rcx                  ; r9 = remaining width

    ; Skip leading spaces
.skip_spaces:
    cmp r9, 0
    jle .no_number
    movzx rax, byte [r8]
    cmp al, ' '
    jne .found_digit_start
    inc r8
    dec r9
    jmp .skip_spaces

.found_digit_start:
    ; Check if it's a digit
    sub al, '0'
    cmp al, 9
    ja .no_number

    ; Parse the number
    xor rax, rax
.parse_loop:
    cmp r9, 0
    jle .got_number
    movzx rcx, byte [r8]
    sub rcx, '0'
    cmp rcx, 9
    ja .got_number
    imul rax, 10
    add rax, rcx
    inc r8
    dec r9
    jmp .parse_loop

.got_number:
    mov rbx, 1
    pop r9
    pop r8
    ret

.no_number:
    xor rax, rax
    xor rbx, rbx
    pop r9
    pop r8
    ret

section .data
filename: db "../inputs/day6_input.txt", 0

section .bss
file_size: resq 1
line_width: resq 1
line_stride: resq 1
num_lines: resq 1
accumulator: resq 1
current_op: resb 1
